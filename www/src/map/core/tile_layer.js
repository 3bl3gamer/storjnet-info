export function MapTileLayer(initialTileHost) {
	var tileHost
	...
	this._updateDrawParams = function (map) {
		...
		draw_i_numb = Math.min(level_grid_width-draw_i_from, ((map.canvas.dp_width -draw_x_shift)/block_size|0)+1) //prettier-ignore
		draw_j_numb = Math.min(level_grid_width-draw_j_from, ((map.canvas.dp_height-draw_y_shift)/block_size|0)+1) //prettier-ignore
	...
	this._drawOneTile = function (map, x, y, scale, i, j) {
		...
		if (!drawed) {
			...
			if (!drawed) {
				tileHost.tryDrawAsQuarter(map,this, x,y,scale, 0,0, i*2  ,j*2  , level+1) //prettier-ignore
				tileHost.tryDrawAsQuarter(map,this, x,y,scale, 0,1, i*2  ,j*2+1, level+1) //prettier-ignore
				tileHost.tryDrawAsQuarter(map,this, x,y,scale, 1,0, i*2+1,j*2  , level+1) //prettier-ignore
				tileHost.tryDrawAsQuarter(map,this, x,y,scale, 1,1, i*2+1,j*2+1, level+1) //prettier-ignore
			}
		}
	}
